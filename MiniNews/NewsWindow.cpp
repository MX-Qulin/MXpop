...
LRESULT CNewsWindow::OnControlClick(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if(nControlIndex==NCT_CLOSE){
		...
		//	PostMessage(m_hWnd, WM_CLOSE, NULL, NULL);
			...
			nid.uCallbackMessage = (WM_USER + 2);//自定义的消息 处理托盘图标事件
		//	nid.hIcon = LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDB_SKIN_QQ));
			nid.hIcon = MXIcon;
			wcscpy_s(nid.szTip, _T("MX-Codingland下载"));//鼠标放在托盘图标上时显示的文字
